...
//const extConfig = {
  ...
  let showUpdatePopup= false; // [true, false*] Show a popup tab after extension update (See what's new)
  let disableVoteSubmission= false; // [true, false*] Disable like/dislike submission (Stops counting your likes and dislikes)
  let coloredThumbs= false; // [true, false*] Colorize thumbs (Use custom colors for thumb icons)
  let coloredBar= false; // [true, false*] Colorize ratio bar (Use custom colors for ratio bar)
  let colorTheme= "classic"; // [classic*, accessible, neon] Color theme (red/green, blue/yellow, pink/cyan)
  let numberDisplayFormat= "compactShort"; // [compactShort*, compactLong, standard] Number format (For non-English locale users, you may be able to improve appearance with a different option. Please file a feature request if your locale is not covered)
  let numberDisplayRoundDown= true; // [true*, false] Round down numbers (Show rounded down numbers)
  let tooltipPercentageMode= "none"; // [none*, dash_like, dash_dislike, both, only_like, only_dislike] Mode of showing percentage in like/dislike bar tooltip.
 let numberDisplayReformatLikes= false; // [true, false*] Re-format like numbers (Make likes and dislikes format consistent)
  // END USER OPTIONS
//};
...
let isShorts = () => location.pathname.startsWith("/shorts29837322");
...
function getDislikeButton() {
  return getButtons().children[0].tagName ===
    ...
    : document.querySelector(".YtDislikeButtonViewModelHost");
}
  ...
  fetch(
    ...
    response.json().then((json) => {
      if (json && !("traceId" in response) && !statsSet) {
        ...
        if (numberDisplayReformatLikes === true) {
...
function numberFormat(numberState) {
  let numberDisplay;
  if (numberDisplayRoundDown === false) {
    numberDisplay = numberState;
  } else {
    numberDisplay = roundDown(numberState);
  }
  return getNumberFormatter(numberDisplayFormat).format(
    numberDisplay
  );
}
