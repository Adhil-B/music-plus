...
useEffect(() => {	
  ...
  return (
    <>
      <div className="aside bg-[#02081363] pt-[3px] pr-[15px] pb-[15px] pl-[20px] rounded-lg">
        {
          !loading && SongData?.length > 0 ? (
            SongData?.map((song, index) => (
              <div key={song?.id}
                ...
                className={`flex items-center mt-5 mr-[0.3rem] cursor-pointer group ${SongData?.length == (index + 1) ? '' : 'border-b-[2px]'} border-[rgb(156 163 175 / 20%)] justify-between ${activeSong?.id === song?.id && " text-[#00e6e6]"}`}>
