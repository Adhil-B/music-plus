...
const FullscreenTrack = ({ fullScreen, activeSong, handlePrevSong, handleNextSong, scrollableDivRef, currentSongs, scrollPosition, handleAddToFavourite }) => {
  ...
  const handlers = useSwipeable({
    onSwipedLeft: () => {setSwipe('motion-preset-slide-right'); setTimeout(() => {handleNextSong(); setSwipe('');}, 400); },
    onSwipedRight: () => {setSwipe('motion-preset-slide-left'); setTimeout(() => {handlePrevSong(); setSwipe('');}, 400); },
  ...
  return (
    <div {...handlers1} onDoubleClick={onDoubleClickHandler} className={`${fullScreen ? 'block motion-preset-slide-up' : 'motion-preset-fade hidden'} w-[100%] flex lg:flex-row lg:w-[100vw] mx-auto flex-col  lg:justify-between sm:mt-10`}>
