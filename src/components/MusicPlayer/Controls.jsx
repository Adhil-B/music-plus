    ...
    <div className={`flex items-center justify-around md:w-80 text-lg lg:w-80 2xl:w-80 gap-4 ${fullScreen ? 'sm:gap-0' : ''}`}>
      ...
      {isPlaying ? (
       ...
       fullScreen ? (
        ...
        <BsFillPauseFill size={45} color={'#00e6e6'} onClick={handlePlayPause} className={`${fullScreen ? '' : 'motion-opacity-in-0 motion-duration-400'} cursor-pointer`} />
        )
        ...
        <BsFillPlayFill size={45} color={'#00e6e6'} onClick={handlePlayPause} className={`${fullScreen ? '' : 'motion-opacity-in-0 motion-duration-400'} cursor-pointer`} />
        )
