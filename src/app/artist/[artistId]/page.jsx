...
//<div className="absolute lg:w-[400px] w-[300px] inset-0 bg-gradient-to-t from-black via-transparent"></div>
    return (
      ...
      <h1 className=" text-3xl font-bold mt-4 mb-4">Songs</h1>
        ...
        <div onClick={songPrv} className={` m-4 mb-5 border ${songsPage !== 1 ? 'border-white' : 'border-zinc-500'} rounded-full cursor-pointer`}>
      <MdNavigateBefore size={35}  className={`${songsPage !== 1 ? 'text-white' : 'text-zinc-500'} text-xl`} />
      </div>
      <div onClick={songNext} className={` m-4 mb-5 border ${!lastPage ? 'border-white' : 'border-zinc-500'} rounded-full cursor-pointer`}>
      <MdNavigateNext size={35}  className={`${!lastPage ? 'text-white' : 'text-zinc-500'} text-xl`} />
      </div>
      </div>
      </div>
                {
